...
<li >                           
    <a href="{{url('/users')}}"><i class="fa fa-users"></i> <span>Pengguna</span></a>
  </li>
  <li><a><i class="fa fa-home"></i> <span>Menu </span><span class="fa fa-angle-down"></span></a>
    <ul class="nav child_menu">
      <li><a href="{{url('/menu')}}"><span>Makanan & Minuman</span></a></li>
      <li><a href="{{url('/category')}}"><span>Kategori</span></a></li>
    </ul>
  </li>

  <li><a href="{{url('/list-menu')}}" ><i class="fa fa-shopping-cart"></i> <span>Tambah Orderan</span></a>
  </li>

  <li><a href="{{url('/order')}}" ><i class="fa fa-shopping-bag"></i> <span>Order</span></a>
  </li>
  <li class="active"><a><i class="fa fa-dollar"></i> <span>Transaksi </span><span class="fa fa-angle-down"></span></a>
    <ul class="nav child_menu">
      <li><a href="#"><span>Pembayaran</span></a></li>
      <li><a href="{{url('/history')}}"><span>Histori</span></a></li>
    </ul>
  </li>

    <li><a href="{{url('/reports')}}" ><i class="fa fa-file"></i> <span>Laporan</span></a>
    </li>  
    <li><a href="{{url('/testimony')}}" ><i class="fa fa-star"></i> <span>Testimoni</span></a>
...
<li class="active"><a><i class="fa fa-dollar"></i> <span>Transaksi </span><span class="fa fa-angle-down"></span></a>
  <ul class="nav child_menu">
    <li><a href="{{url('/payment')}}"><span>Pembayaran</span></a></li>
    <li><a href="{{url('/history')}}"><span>Histori</span></a></li>
  </ul>
</li>
          ...
          <div id="dashboard-v1" class="right_col" role="main">
            ...
            <div class="panel panel-default element-box-shadow">
              <div class="panel-heading padding_30">
                <h3 class="no-margin"> <i class="fa fa-dollar"></i> Pembayaran</h3> <br>
                  ...
                  <thead>
                     <tr>
                       @if (Auth::user()->level == "1")
                        ...
                        <th>Pengguna <i class="fa fa-sort"></i></th>
                   ...
                   <tbody>
                     @foreach (\App\Order::with(['transaksi', 'meja', 'detail_order'=>function($detail_order){
                      ...
                      }])->orderBy('id','desc')->where('status', 'belum dibayar')->orwhere('status', 'belum dibayar')->get() as $i => $order)
                      ...
                      <tr>
                          ...
                          @if (Auth::user()->level == "4")
                                ...
                                <td><label for="" class="label label-success">Sudah dibayar</label></td>
                                @endif
                                @if ($order->transaksi == null) 
                                <td><label for="" class="label label-success">Belum dibayar</label></td>
                                @endif
                          @endif
                          <td>  
                              @if (Auth::user()->level =="1")
                                @if ($order->transaksi != null) 
                                  @if (Auth::user()->level != "2")                                    
                                  <a href="{{ url('/order/invoice/' . $order->id) }}" class="btn btn-sm btn-success">Cetak</a>
                                  @endif
                                @else 
                                ...
                                <form action="{{ url('order/bayar/' . $order->id) }}" method="POST">
                                  ...
                                  <button type="submit" href="" class="btn btn-success">Bayar</button>
                                </form>
                                </div>
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header" style="background-color:#ffe139;">
                  <h2 class="modal-title text-center" id="addModalLabel"><i class="fa fa-plus"></i> Tambah Meja</h2>
                ...
                <div class="modal-body">
                  <form action="{{url('/table/store')}}" method="post">
                    ...
                    <input name="name" type="text" class="form-control" placeholder="Contoh : Meja1">
                ...
                <div class="modal-footer">
                  ...
                  <button type="submit" class="btn btn-success" style="background-color:#ffe139; color:#000;">Simpan</button>
                </div>
              </form>
...
<script>
    ...
    swal('Failed', '{{ session('msgError') }}', 'error');
    @endif
</script>
@endsection